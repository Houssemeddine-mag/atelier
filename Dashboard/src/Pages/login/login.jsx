  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const user = await axios.get(
        import.meta.env.VITE_API_BASE_URL + "/v1/auth/user",
        {
      ...
      navigate(roleMap[user.data.role.toLowerCase()] || "/");
    } catch (err) {
      setError(
    ...
    <div className={C.login}>
      <div className={C.loginform}>
        ...
        <form className={C.form} onSubmit={handleLogin}>
          ...
          <button type="submit" className={C.loginButton} disabled={isLoading}>
            {isLoading ? (
              <span className={C.loadingText}>Logging in...</span>
            ) : (
              "Log In"
            )}
          </button>
        ...
        <div className={C.copy}>
          <p className={C.copymsg}>
            &copy; FeedMe Restaurant Management. All rights Reserved
          </p>
        </div>
      </div>
    </div>
